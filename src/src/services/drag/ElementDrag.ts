 ...
 * 要素ドラッグ管理
 */
export default class ElementDrag {
  private drop;
  ...
  constructor(drop: HTMLElement) {
    ...
    this.drop.addEventListener('dragover', this.onDragOverElement);
    this.drop.addEventListener('drop', this.onDropElement);
  }

  /** ドラッグ開始時 */
  onDragStartElement = (e: DragEvent) => {
  ...
  /** ドラッグしているとき */
  private onDragOverElement = (e: DragEvent) => {
    console.log('onDragOverElement');
    e.preventDefault(); // ドロップ無効を無効化する
  };

  /** ドロップ時 */
  private onDropElement = (e: DragEvent) => {
   ...
   */
  private setDragImage = (e: DragEvent) => {
